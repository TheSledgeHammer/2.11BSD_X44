...
#include "newsgate.h"
#ifdef	SYSLOG
# include "../../../sys/sys/syslog.h"
# undef stderr
# define	fprintf		syslog
# define	stderr		LOG_NOTICE
#endif	SYSLOG

#ifdef	RCSIDENT
static char rcsid[] = "$Header: /usr/local/src/usenet/notes/src/RCS/newsinput.c,v 1.8 88/11/11 00:58:33 paul Exp $";
#endif	RCSIDENT
...
static int  ShowTruncated = 1;				/* log truncated id's */
static int  JustFirstGroup = 0;				/* into only 1 group */
...
#ifdef	SYSLOG
    openlog ("newsinput", LOG_PID, SYSLOG);
#endif	SYSLOG
...
/*
 *	run down the arg vector. looking for various procesing options.
 *
 *	-t	do not log "truncated message id" situations in nfmaint.
 *	-1	for multi-newsgroup articles, save this in the first
 *		group listed and none of the others.
 */

    for (i = 1; i < argc; i++)
    {
	if (argv[i] == (char *) NULL)			/* done */
	    break;
	if (strcmp (argv[i], "-t") == 0)		/* suppress truncation msgs */
	{
	    ShowTruncated = 0;
	    continue;
	}
	if (strcmp (argv[i], "-1") == 0)		/* just first group */
	{
	    JustFirstGroup++;
	    continue;
	}
 ...
 */
    ...
    {
	fprintf (stderr, "Incoming News mangled more than usual\n");
	exit (BAD);
    }
 ...
 */
    ...
    {
	fprintf (stderr, "Control message (ignored): %s\n", header.ctlmsg);
	exit (0);					/* "success" */
    }
/*
 ...
 */
    ...
    {
	fprintf (stderr, "Had problems creating/opening file %s\n", pathname);
	exit (BAD);					/* die */
    }
 ...
 */
	...
	if (tail != (char *) NULL && !strcmp (tail, CTL))/* it is one */
	{
	    ...
	    /* 
	     * re-route the control message to NFMAINT.
	     */
	...
	if (init (&io, nf) < 0)				/* open the nf */
	{
...
#ifdef AUTOCREATE
	    /* 
	     * create the notesfile; we are allowed to do this.
	     */
	    sprintf (pbuf,
	    ...
	    {
		/* 
		 * could be a truncation error; mark that in the message
		 */
...
#else
	    /* 
	     * drop the article, log it in nfmaint
	     */
	    sprintf (pbuf,
		    "Notesfile: %s\nNewsgroup %s\n\nNOT created by newsinput\n",
		    nf, nfptr -> nf_name);
	    sprintf (tbuf, "No Such NF: %s", nf);
	    nfcomment (NFMAINT, pbuf, tbuf, 0, 0);
	    fclose (body);				/* close article */
	    goto nextstep;
#endif AUTOCREATE
	}
	...
	finish (&io);
nextstep: 
	if (JustFirstGroup)				/* only wanted 1 */
	{
	    break;					/* so exit this loop */
	}
    }
 ...
 */
    ...
    if (suffix != (char *) NULL &&			/* have suffix */
	    (!strcmp (suffix, NFSUFFIX) || !strcmp (suffix, OLDSUFFIX)))
    {							/* and it matches */
 ...
 */
	...
	{
	    if (hline1[0] == '#' &&
		    (hline1[1] == 'N' || hline1[1] == 'R'))
	    {
		break;					/* found one */
	    }
	}
	/* 
	 * could probably have looked for "#N:" and "#R:". i
	 * think that the old style lines had those colons.
	 * ok; we either EOF'ed or found an appropriate line
	 */
	if (hline1[0] != '#' ||
		(hline1[1] != 'N' && hline1[1] != 'R'))	/* actually didn't */
	{						/* it wasn't really a #N or #R line */
 ...
 */
	...
	case 'N': 					/* base note */
		...
		dprintf ("Orphaned response chain adopted\n");
		return (notenum);			/* note number */
	    }
	    else
		dprintf ("Duplicate note handed back by news\n");
	    unlocknf (io, DSCRLOCK);
	...
	case 'R': 					/* response */
	    ...
	    {						/* build foster */
		dprintf ("Orphaned response handed in by news\n");
 ...
 */
		...
		dprintf ("Duplicate response handed back by news\n");
	    unlocknf (io, DSCRLOCK);
	    return (0);					/* resolved */
...
FILE * body;
{
    ...
    {
	fprintf (stderr, "System %s not allowed to write notes\n", origsys);
	return (0);					/* NO! */
    }
    ...
    /* 
     * new tricks with saving the entire message id in the
     * system portion of the id_f structure
     */
    if (i < 2 && index (header -> ident, ':') == (char *) NULL)
    {							/* not unpacked yet */
	newsseq = strlen (header -> ident);		/* save length */
	if (newsseq < SYSSZ)				/* all fits */
	{
	    strcpy (field1, header -> ident);		/* entire id */
	}
	else
	{						/* too long */
#ifdef	NFMAINT
	    ...
	    strncpy (field1, header -> ident, SYSSZ);
	    field1[SYSSZ - 1] = '\0';			/* truncate */
	    dprintf ("Chopped %d byte message id '%s' to '%s'\n",
		    newsseq, header -> ident, field1);
#ifdef	NFMAINT
	    if (ShowTruncated)				/* log it */
	    {
		sprintf (pbuf,
			"%d byte Message-ID: '%s' truncated to '%s'\nPath: %s\nFrom: %s\nNewsgroups: %s\n",
			newsseq, header -> ident, field1,
			header -> path, header -> from, header -> nbuf);
		nfcomment (NFMAINT, pbuf, "Truncated Message-ID", 0, 0);
	    }
#endif	NFMAINT
	}
	newsseq = -newsseq;				/* make it < 0 */
	i = 2;						/* mark done */
    }
    if (i < 2)						/* no id */
    {
	...
	nfcomment (NFMAINT, pbuf, "UnManageable Message-ID", 0, 0);
#endif	NFMAINT
	dprintf ("can't fathom Message-ID: %s\n", header -> ident);
	return (-1);					/* mark bogus */
    }
 ...
 */
	...
	{
	    dprintf ("Duplicate news article received\n");
