  ...
  int *uid;
{
  ...
  if (!getbuf(nl[N_NFILE].n_value, &nfile_dummy, sizeof(nfile_dummy), "nfile"))
    return -1;
  if (!getbuf(nl[N_FILE].n_value, &addr, sizeof(addr), "&file"))
    return -1;

  /* Convert pointer nfile_dummy to an integer number of files */
  nfile = (int) ((nfile_dummy - addr) / sizeof(struct file));
