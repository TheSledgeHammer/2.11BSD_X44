...
int k_open()
{
  ...
  */
  if ((rcode = kvm_nlist(kd, nl)) != 0)
      if (rcode != 3)
	  ERROR1("main: kvm_nlist, rcode = %d", rcode);
...
{
  struct inpcb *head;
  int limiter = 65536;	/* avoid infinite loop <umura@nn.solan.chubu.ac.jp> */
  ...
  do 
  {
    ...
    if (--limiter <= 0)	/* avoid infinite loop <umura@nn.solan.chubu.ac.jp> */
        break;
