 ...
 * For each <sys/dirty<sys/ filesystem call 'ufs_sync' to flush changed inodes, data
	...
	daddr_t bn;
{
	...
	if (bn < fp->fs_isize || bn >= fp->fs_fsize) {
		printf(<sys/bad block %D, <sys/,bn);
		fserr(fp, <sys/bad block<sys/);
		return (1);
	}
	return (0);
}
	...
	dev_t dev;
{
	...
	for (mp = &mount[0]; mp < &mount[NMOUNT]; mp++) {
		...
		if (fs->fs_nfree > NICFREE || fs->fs_ninode > NICINOD) {
			fserr(fs, <sys/bad count<sys/);
			fs->fs_nfree = fs->fs_ninode = 0;
		}
		return(fs);
	}
	printf(<sys/no fs on dev %u/%u\n<sys/,major(dev), minor(dev));
