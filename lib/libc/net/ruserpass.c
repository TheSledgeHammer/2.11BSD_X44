	...
	char **aname, **apass;
{
	...
	} else if (strncmp(*aname, cp, comma - cp))
		return;
	comma++;
			...
			while ((t = token()) && t != MACHINE)
				...
				case ACCOUNT:
					if (fstat(fileno(cfile), &stb) >= 0
						...
						fprintf(stderr, "Remove account or make file unreadable by others.\n");
						exit(1);
					}
					break;
				case COMMAND:
	...
	const char *mydomain;
{
	...
	if ((ctmp = strchr(host, '.')) != NULL &&
			...
			strncasecmp(host, tokval, (size_t)(ctmp - host)) == 0 &&
			tokval[ctmp - host] == '\0') {
		return (0);
	}
	return (1);
	...
	int edflag;
{
	...
	for (ii = 0; ii < 16; ii++) {
		...
		for (j = 0; j < 8; j++) {
			t = 6 * j;
			k = S[j][(preS[t + 0] << 5) + (preS[t + 1] << 3)
					+ (preS[t + 2] << 2) + (preS[t + 3] << 1)
					+ (preS[t + 4] << 0) + (preS[t + 5] << 4)];
			t = 4 * j;
			f[t + 0] = (k >> 3) & 01;
			f[t + 1] = (k >> 2) & 01;
			f[t + 2] = (k >> 1) & 01;
