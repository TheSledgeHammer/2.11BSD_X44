...
struct file {
	...
	struct lfs		*f_lfs;					/* pointer to super-block */
	...
	struct open_file *f;
{
	 ...
	 */
	buf = alloc(fs->lfs_bsize);
	rc = (f->f_dev->dv_strategy)(f->f_devdata, F_READ,
		fsbtodb(fs, ino_to_fsba(fs, inumber)), fs->lfs_bsize,
		buf, &rsize);
	if (rc)
		goto out;
	if (rsize != fs->lfs_bsize) {
		rc = EIO;
		goto out;
	}
	...
	for (; level >= 0; level--) {
		...
		if (fp->f_blkno[level] != ind_block_num) {
			if (fp->f_blk[level] == (char*) 0)
				fp->f_blk[level] = alloc(fs->lfs_bsize);
			rc = (f->f_dev->dv_strategy)(f->f_devdata, F_READ,
					fsbtodb(fp->f_lfs, ind_block_num), fs->lfs_bsize,
					fp->f_blk[level], &fp->f_blksize[level]);
			if (rc)
				return (rc);
			if (fp->f_blksize[level] != fs->lfs_bsize)
				return (EIO);
	...
	ino_t *inumber_p;		/* out */
{
	...
	while (fp->f_seekp < DIP(fp->f_ip, size)) {
		...
		while (dp < edp) {
			...
			if (fp->f_lfs->lfs_maxsymlinklen <= 0)
				namlen = dp->d_type;
			else
#endif
				namlen = dp->d_namlen;
			...
			next: dp = (struct direct*) ((char*) dp + dp->d_reclen);
		}
		fp->f_seekp += buf_size;
	}
	return (ENOENT);
	...
	struct open_file *f;
{
	...
	fs = alloc(LFS_SBPAD);
	fp->f_lfs = fs;
	rc = (f->f_dev->dv_strategy)(f->f_devdata, F_READ, SBLOCK, LFS_SBPAD, (char *)fs, &buf_size);
	if (rc)
		goto out;
	...
	struct open_file *f;
{
	...
	for (level = 0; level < NIADDR; level++) {
		if (fp->f_blk[level]) {
			free(fp->f_blk[level], fp->f_lfs->lfs_bsize);
		}
	}
	if (fp->f_buf) {
		free(fp->f_buf, fp->f_lfs->lfs_bsize);
	}
	free(fp->f_lfs, LFS_SBPAD);
	free(fp, sizeof(struct file));
	return (0);
}
