...
pnp_scan(int argc, char *argv[])
{
	...
	if (verbose) {
		...
		if (pager_output("PNP scan summary:\n"))
			goto out;
		TAILQ_FOREACH(pi, &pnp_devices, pi_link)
		{
			pager_output(TAILQ_FIRST(&pi->pi_ident)->id_ident); /* first ident should be canonical */
			if (pi->pi_desc != NULL) {
				pager_output(" : ");
...
pnp_discard(void)
{
	...
	while (STAILQ_FIRST(&pnp_devices) != NULL) {
		...
		TAILQ_REMOVE(&pnp_devices, pi, pi_link);
		pnp_freeinfo(pi);
	}
}
...
pnp_freeinfo(struct pnpinfo *pi)
{
	...
	while (!TAILQ_EMPTY(&pi->pi_ident)) {
		id = TAILQ_FIRST(&pi->pi_ident);
		TAILQ_REMOVE(&pi->pi_ident, id, id_link);
		free(id->id_ident);
		free(id);
	}
	if (pi->pi_desc)
...
{
    TAILQ_INSERT_TAIL(&pnp_devices, pi, pi_link);
}
