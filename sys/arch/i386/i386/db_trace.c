...
int db_numargs(int *);
int db_nextframe(int **, int **, int **, db_addr_t *, int *, int, void (*) (const char *, ...));
		     ...
		     char *modif, void (*pr)(const char *, ...))
{
	...
	if (!have_addr) {
		...
		if (trace_thread) {
			...
			if (p == curproc && l == curlwp) {
				...
				callpc = (db_addr_t) db_get_value((int) (frame + 1), 4, FALSE);
				(*pr)(" at %p\n", frame);
	...
	while (count && frame != 0) {
		...
		is_trap = NONE;
		offset = 0;
		sym = db_frame_info(frame, callpc, &name, &offset, &is_trap, &narg);
		...
		if (lastframe == 0 && offset == 0 && !have_addr) {
			...
			lastframe = (int*) fp;
			callpc = (db_addr_t) db_get_value((db_addr_t) & fp->f_retaddr, 4,
					FALSE);
			continue;
		}
		...
		lastframe = frame;
		lastcallpc = callpc;
		if (!db_nextframe(&frame, &retaddr, &arg0, &callpc, frame + 2, is_trap,
				pr))
			break;
