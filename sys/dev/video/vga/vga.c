...
struct egavga_font *egavga_getfont(struct vga_config *, struct vgascreen *, const char *, int);
void egavga_unreffont(struct vga_config *, struct egavga_font *);

int vga_selectfont(struct vga_config *, struct vgascreen *, const char *, const char *);
void vga_init_screen(struct vga_config *, struct vgascreen *, const struct wsscreen_descr *, int, long *);
...
const struct wsscreen_list vga_screenlist = {
	...
	sizeof(_vga_scrlist_mono) / sizeof(struct wsscreen_descr *), _vga_scrlist_mono
};
...
vga_init(struct vga_config *vc, bus_space_tag_t iot, bus_space_tag_t memt)
{
	...
	if (vga_no_builtinfont) {
		...
		cookie = wsfont_find(NULL, 8, 16, 0, WSDISPLAY_FONTORDER_L2R, 0);
