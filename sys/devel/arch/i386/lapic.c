...
/* XXX */
extern int bus_mem_add_mapping(bus_addr_t, bus_size_t, int, bus_space_handle_t *);
...
ioapic_attach(struct device *parent, struct device *self, void *aux)
{
	...
	if (apic_id != sc->sc_apicid) {
		if (mp_verbose)
			printf("\n%s: misconfigured as apic %d", sc->sc_pic.pic_name,
					apic_id);
		ioapic_set_id(sc);
	}
...
ioapic_enable(void)
{
	...
	for (sc = ioapics; sc != NULL; sc = sc->sc_next) {
		...
		for (p = 0; p < sc->sc_apic_sz; p++) {
			...
			for (q = sc->sc_pins[p].ip_handler; q != NULL; q = q->ih_next) {
